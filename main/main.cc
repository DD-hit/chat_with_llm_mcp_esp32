...
#include "mqtt_client.h"            // MQTT 客户端
}
...
// 📡 网络配置（请根据您的实际情况修改）
#define WIFI_SSID "DD"                 // 您的WiFi名称
#define WIFI_PASS "31812449670.luo"           // 您的WiFi密码

// 🌐 WebSocket服务器配置
#define WS_URI "ws://192.168.31.235:8888" // 请改为您的电脑IP地址:8888

// 新增: MQTT 配置
#define MQTT_BROKER_URI "mqtt://192.168.31.235:1883"  // 请改为您的 MQTT Broker 地址
#define MQTT_TOPIC_LED_CMD   "esp32/led/control"
...
// 发布 LED 状态（ON/OFF，大写）
static void mqtt_publish_led_state()
{
    if (!mqtt_client) return;
    int level = gpio_get_level(LED_GPIO);
    const char* state = level ? "ON" : "OFF";
    esp_mqtt_client_publish(mqtt_client, MQTT_TOPIC_LED_STATE, state, 0, 1, 1);
}

// 解析 LED 控制命令（只认 ON/OFF/BLINK，大写）
static void handle_led_command(const char* payload, int len)
{
    // 拷贝
    char buf[16];
    ...
    if (start > 0 && new_len > 0) memmove(buf, buf + start, (size_t)new_len);
    buf[new_len] = '\0';

    // 统一大写化
    for (int i = 0; i < new_len; ++i) buf[i] = (char)toupper((unsigned char)buf[i]);
    ...
    if (!strcmp(buf, "ON")) {
        ...
        ESP_LOGW(TAG, "MQTT 未识别的LED命令: %s", buf);
    }
}

// MQTT 事件处理（topic判断方式调整为精确匹配）
static esp_err_t mqtt_event_handler_cb(esp_mqtt_event_handle_t event)
{
    switch (event->event_id) {
    case MQTT_EVENT_CONNECTED:
        ESP_LOGI(TAG, "🔗 MQTT 连接成功，订阅: %s", MQTT_TOPIC_LED_CMD);
        esp_mqtt_client_subscribe(event->client, MQTT_TOPIC_LED_CMD, 1);
        mqtt_publish_led_state();
        break;
    ...
    case MQTT_EVENT_DATA:
        ESP_LOGI(TAG, "📩 收到MQTT消息: topic=%.*s, data=%.*s",
                 event->topic_len, event->topic, event->data_len, event->data);
        if (event->topic && strncmp(event->topic, MQTT_TOPIC_LED_CMD, event->topic_len) == 0
            && strlen(MQTT_TOPIC_LED_CMD) == (size_t)event->topic_len) {
...
extern "C" void app_main(void)
{
    ...
    // 新增: 启动 MQTT（依赖已连接的 WiFi）
    ESP_LOGI(TAG, "正在连接MQTT Broker...");
