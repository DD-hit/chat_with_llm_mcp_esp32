...
/**
 * @brief WebSocket事件处理器（静态函数）
 * @param handler_args WebSocketClient实例指针
 * @param base 事件基类
 * @param event_id 事件ID（连接、断开、数据等）
 * @param event_data 事件数据结构体
 * 负责分发和处理WebSocket底层事件。
 */
...
/**
 * @brief 自动重连任务（FreeRTOS任务）
 * @param arg WebSocketClient实例指针
 * 循环检测连接状态，断线时自动重连。
 */
 ...
 * @return ESP_OK成功，其他为失败
 * 初始化并启动WebSocket客户端，注册事件。
 */
